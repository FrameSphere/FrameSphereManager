...
async function handleRequest(request, env) {
  ...
  // ── CF WORKER ANALYTICS (24h) ────────────────────────────────
  if (path === '/api/cf-analytics' && request.method === 'GET') {
    if (!await verifyAuth(request, env)) return err('Unauthorized', 401);
    const cfToken   = request.headers.get('CF-Token') || '';
    const accountId = '75ab77c2ccd4045de59e99835480bc53';
    if (!cfToken) return err('CF-Token header missing', 400);
    const query = `{ viewer { accounts(filter:{accountTag:"${accountId}"}) {
      workersInvocationsAdaptive(limit:10000 filter:{datetimeHour_geq:"${new Date(Date.now()-86400000).toISOString().slice(0,13)}:00:00Z"}) {
    ...
    try {
      ...
      const d = await r.json();
      const inv = d?.data?.viewer?.accounts?.[0]?.workersInvocationsAdaptive?.[0];
      if (!inv) return json({ requests:0, errors:0, subrequests:0, cpuP50:0, cpuP99:0 });
      return json({
        requests:    inv.sum?.requests    || 0,
